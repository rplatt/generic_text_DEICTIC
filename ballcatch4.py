...
#            targetsItemLast = rewards + (1-dones) * gamma * qNextmax
#            targetsTiled = np.tile(np.reshape(targetsItemLast,[batch_size,1]),[1,num_deictic_patches])
#            tabularQ[stateCurr[0,:], stateCurr[1,:], stateCurr[2,:], stateCurr[3,:],actions] = np.minimum(targetsTiled, tabularQ[stateCurr[0,:], stateCurr[1,:], stateCurr[2,:], stateCurr[3,:],actions])
...
#                if targetsItemLast != targetsItem:
#                    print("!!!!!!!!!!!!!!!!! ERROR !!!!!!!!!!!!!!!!!!")
...
#        qNext = tabularQ[stateNext[0,:], stateNext[1,:], stateNext[2,:], stateNext[3,:],:]
#        qNextmax = np.max(qNext)
#        targetsItem = rewards + (1-dones) * gamma * qNextmax
#            targetsTiled = np.tile(np.reshape(targetsItem,[batch_size,1]),[1,num_deictic_patches])
#            tabularQ[stateCurr[0,:], stateCurr[1,:], stateCurr[2,:], stateCurr[3,:],action] = np.minimum(targetsTiled, tabularQ[stateCurr[0,:], stateCurr[1,:], stateCurr[2,:], stateCurr[3,:],action])
            
#            print("targetsItemLast: " + str(targetsItemLast))
#            print("targetsItem:     " + str(targetsItem))
