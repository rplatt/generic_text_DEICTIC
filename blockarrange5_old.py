#
# Implementation of the DQN version of the cascade.
#
# Adapted from blockarrange4.py
#
#
import gym
import numpy as np
...
#        actionDescriptors = np.r_[actionsPickDescriptors,actionsPlaceDescriptors]
        ...
        # TABULAR version
#        actionDescriptorsTabular = np.reshape(actionDescriptors,[-1,deicticActionShape[0]*deicticActionShape[1]*deicticActionShape[2]])==1
        moveDescriptors = np.reshape(moveDescriptors,[-1,deicticShape[0]*deicticShape[1]*deicticShape[2]])
        actionDescriptorsTabular = np.r_[np.c_[np.zeros([num_patches,1])==1,moveDescriptors],np.c_[np.ones([num_patches,1])==1,moveDescriptors]]
        qCurr = getTabular(actionDescriptorsTabular)
        ...
        replay_buffer.add(stateDeictic, actionDescriptorsTabular[action,:], rew, new_obs, float(done))
...
#            actionsNextPickDescriptors = np.concatenate([moveDescriptorsNext,np.zeros(np.shape(moveDescriptorsNext))],axis=3)
#            actionsNextPlaceDescriptors = np.concatenate([np.zeros(np.shape(moveDescriptorsNext)),moveDescriptorsNext],axis=3)
#            actionsNextPickDescriptors = np.concatenate([np.zeros(np.shape(moveDescriptorsNext)),moveDescriptorsNext],axis=3)
#            actionsNextPlaceDescriptors = np.concatenate([np.ones(np.shape(moveDescriptorsNext)),moveDescriptorsNext],axis=3)
#            actionNextDescriptors = np.r_[actionsNextPickDescriptors,actionsNextPlaceDescriptors]

            # TABULAR version
            moveDescriptorsNext = np.reshape(moveDescriptorsNext,[-1,deicticShape[0]*deicticShape[1]*deicticShape[2]])
            actionNextDescriptorsTabular = np.r_[np.c_[np.zeros([batch_size*num_patches,1])==1,moveDescriptorsNext],np.c_[np.ones([batch_size*num_patches,1])==1,moveDescriptorsNext]]
#            actionNextDescriptorsTabular = np.reshape(actionNextDescriptors,[-1,deicticActionShape[0]*deicticActionShape[1]*deicticActionShape[2]])==1
            qNext = getTabular(actionNextDescriptorsTabular)

            # DQN version
#            qNext = getq(actionNextDescriptors)
            ...
            # TABULAR version
#            actionsTabular = np.reshape(actions,[-1,deicticActionShape[0]*deicticActionShape[1]*deicticActionShape[2]])==1
            actionsTabular = actions
            qCurr = getTabular(actionsTabular)
...
#            qCurr = getq(actions)
            ...
            # TABULAR version
            trainTabular(actionsTabular,qCurrTarget)
