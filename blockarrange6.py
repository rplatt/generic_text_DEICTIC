#
# This is a one-channel version of blockarrange5.py. There are only two states: 0, 1 (holding, not holding)
# The patch observation only contains 0 or 1 -- although there are two blocks,
# the system does not differentiate between them.
#
# Adapted from blockarrange5.py
#
# Results: this version seems to work just as well as any of the prior versions. Haven't tested in the tabular case...
#
import gym
import numpy as np
import time as time
...
#        moveDescriptors = 10*moveDescriptors[:,:,:,0] + 20*moveDescriptors[:,:,:,1]
        moveDescriptors = moveDescriptors[:,:,:,0] + moveDescriptors[:,:,:,1]
...
#            moveDescriptorsNext1 = 10*moveDescriptorsNext1[:,:,:,0] + 20*moveDescriptorsNext1[:,:,:,1]
            moveDescriptorsNext1 = moveDescriptorsNext1[:,:,:,0] + moveDescriptorsNext1[:,:,:,1]
