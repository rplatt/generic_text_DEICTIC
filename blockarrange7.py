#
# A variation on blockarrange6.py where I encode block interchangability in 
# getMoveActionDescriptors. This version learns to place three blocks in a row.
# Uses blockarrange2_rewardonsuccess_standalone.py as the env.
#
# This version also allows you to select from two different types of action selection.
# The first selects from the total set of actions. The second selects from actions
# w/ unique action descriptors.
#
# Adapted from blockarrange6.py
#
# Results: This basically works for three blocks in a row. I was a bit surprised
#          that it learned so quickly b/c the odds of placing three blocks in a row
#          at random seems quite small...
#
#          -- it seems that action selection from unique action descriptors seems
#             to help a bit, but not as much as I might have expected.
#
#          -- I think adding more hidden nodes can help. I can achieve >9.0 on this task, 
#             but only w/ two conv layers.
#
#          -- Interestingly, this simply doesn't work w/ a tabular representation -- it just 
