#
# This modifies blockarrangeredo5 by adding the cascading value estimate.
# (blockarrangeredo5 is avg value...)
#
# Adapted from blockarrangeredo5.py
#
# Results: I was only able to get this to work by setting all elements of the cascade
# to the same target values. Somehow, the multiple cascade values seem to impede
# convergence. Going to try separate cascaded networks next...
#
#
#
import gym
...
#    memory_t[0,:,:,2] = (env.pickBlockGoal + 2) * np.ones([memoryShape[1], memoryShape[2]]) # DEBUG
    ...
    for t in range(max_timesteps):
        ...
        qCurr = np.stack([qCurrNotHolding,qCurrHolding],axis=1)
#        qCurr = qCurr[:,:,-1]
        qCurr = qCurr[:,:,0]
...
#            memory_tp1[0,:,:,2] = (env.pickBlockGoal + 2) * np.ones([memoryShape[1], memoryShape[2]]) # DEBUG
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            qNextFlat = np.stack([qNextNotHolding,qNextHolding],axis=1)
#            qNextFlat = qNextFlat[:,:,-1]
            qNextFlat = qNextFlat[:,:,0]
            ...
            qCurrTarget = np.stack([qCurrTargetNotHolding,qCurrTargetHolding],axis=1)
            qCurr = np.copy(qCurrTarget)
#            td_error = qCurrTarget[range(batch_size),statesDiscrete_t] - targets

#            # avg value
#            qCurrTarget[range(batch_size),statesDiscrete_t] = targets
#            targetTrainNotHolding(stateActionsImage_t, np.reshape(qCurrTarget[:,0],[batch_size,1]), np.reshape(weights,[batch_size,1]))
#            targetTrainHolding(stateActionsImage_t, np.reshape(qCurrTarget[:,1],[batch_size,1]), np.reshape(weights,[batch_size,1]))

            # cascaded value
            qCurrTarget[range(batch_size),statesDiscrete_t,0] = targets
            for i in range(num_cascade-1):
                qCurrTarget[range(batch_size),statesDiscrete_t,i+1] = targets
#                maskIdx = np.nonzero(targets < qCurr[range(batch_size),statesDiscrete_t,i])[0]
#                if len(maskIdx) > 0:
#                    qCurrTarget[maskIdx,statesDiscrete_t[maskIdx],i+1] = targets[maskIdx]
                    
            targetTrainNotHolding(stateActionsImage_t, qCurrTarget[:,0,:], np.ones([batch_size,num_cascade]))
            targetTrainHolding(stateActionsImage_t, qCurrTarget[:,1,:], np.ones([batch_size,num_cascade]))
...
#            new_obs = env.reset()
    ...
    for i in range(3):
        ...
        print("***")
        print(str(np.reshape(qPickNotHolding[:,1],[8,8])))
        print("***")
        print(str(np.reshape(qPickNotHolding[:,2],[8,8])))
    
        print("\nValue function for look action in hold-nothing state:")
        print(str(np.reshape(qLookNotHolding[:,0],[8,8])))
        print("***")
        print(str(np.reshape(qLookNotHolding[:,1],[8,8])))
        print("***")
        print(str(np.reshape(qLookNotHolding[:,2],[8,8])))
