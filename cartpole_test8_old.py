...
#
# This version does not rely on build_graph -- we buid the neural network in-line.
#
import gym
import numpy as np
...
#    inputDims = [50,2]
    ...
    act2, getq, train2 = build_graph.build_train_robtest(
        make_obs_ph=make_obs_ph,
        q_func=q_func,
        num_actions=env.action_space.n,
        optimizer=tf.train.AdamOptimizer(learning_rate=lr),
...
#    getq = build_graph.build_getq_robtest(make_obs_ph=make_obs_ph,q_func=q_func,num_actions=env.action_space.n)
...
#        q_values_rob = getq(np.array(obs)[None])
...
#            td_errors = train2(obses_t, actions, rewards, obses_tp1, dones, weights)
#            roberrors = train2(obses_t)
            roberrors = train2(obses_t, actions, rewards)
