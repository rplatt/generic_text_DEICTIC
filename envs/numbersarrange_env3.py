...
#        self.num_blocks = 4
    ...
    def reset(self, initStride=None):

        # set moveCenters used to init the board
        if initStride == None:
            self.moveCenters = range(self.blockSize/2,self.gridSize-(self.blockSize/2)+1,self.stride)
        else:
            self.moveCenters = range(self.blockSize/2,self.gridSize-(self.blockSize/2)+1,initStride)
