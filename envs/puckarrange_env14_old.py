...
# 
#
# Derived from puckarrange_env12.py
#
# 
#
#
...
#                            imRot = np.int32(ndimage.rotate(im, 22.5, reshape=False)>0.5)
                            imRot = np.int32(im>0.5)
                        elif orientation == 2:
                            imRot = np.int32(ndimage.rotate(im, 45, reshape=False)>0.5)
                        elif orientation == 3:
#                            imRot = np.int32(ndimage.rotate(im, 67.5, reshape=False)>0.5)
                            imRot = np.int32(ndimage.rotate(im, 45, reshape=False)>0.5)
                        elif orientation == 4:
                            imRot = np.int32(ndimage.rotate(im, 90, reshape=False)>0.5)
                        elif orientation == 5:
#                            imRot = np.int32(ndimage.rotate(im, 112.5, reshape=False)>0.5)
                            imRot = np.int32(ndimage.rotate(im, 90, reshape=False)>0.5)
                        elif orientation == 6:
                            imRot = np.int32(ndimage.rotate(im, 135, reshape=False)>0.5)
                        elif orientation == 7:
#                            imRot = np.int32(ndimage.rotate(im, 157.5, reshape=False)>0.5)
                            imRot = np.int32(ndimage.rotate(im, 135, reshape=False)>0.5)
                        else:
                            print("error in orientation 1")
...
#                if True not in (self.state[0][iiRangeOuter,jjRangeOuter,0] > 0): # if this square is empty
                if True not in (self.state[0][iiRangeOuter,jjRangeOuter,0] > 0.55): # account for the half-points that I sometimes place in a square to indicate target
...
#                        imRot = ndimage.rotate(im, -22.5, reshape=False)
                        imRot = im
                    elif orientation == 2:
                        imRot = ndimage.rotate(im, -45, reshape=False)
                    elif orientation == 3:
#                        imRot = ndimage.rotate(im, -67.5, reshape=False)
                        imRot = ndimage.rotate(im, -45, reshape=False)
                    elif orientation == 4:
                        imRot = ndimage.rotate(im, -90, reshape=False)
                    elif orientation == 5:
#                        imRot = ndimage.rotate(im, -112.5, reshape=False)
                        imRot = ndimage.rotate(im, -90, reshape=False)
                    elif orientation == 6:
                        imRot = ndimage.rotate(im, -135, reshape=False)
                    elif orientation == 7:
#                        imRot = ndimage.rotate(im, -157.5, reshape=False)
                        imRot = ndimage.rotate(im, -135, reshape=False)
                    else:
                        print("error in orientation 2")
        ...
        if self.state[1] == 0: # if both objs on the grid
            im = np.int32(self.state[0][:,:,0]>0.6)
            ...
            im = np.int32(ndimage.rotate(self.state[0][:,:,0],45,reshape=True)>0.6)
            bounds = self.getBoundingBox(im)
