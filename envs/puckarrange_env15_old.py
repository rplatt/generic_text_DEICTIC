...
# 
#
# Derived from puckarrange_env14.py
#
# 
#
#
...
#                    dist2 = dist2+0.01
#                elif randNum < 0.25:
#                    dist2 = ndimage.rotate(dist2+0.01,22.5,reshape=False)
#                elif randNum < 0.375:
#                    dist2 = ndimage.rotate(dist2+0.01,45,reshape=False)
#                elif randNum < 0.5:
#                    dist2 = ndimage.rotate(dist2+0.01,67.5,reshape=False)
#                elif randNum < 0.625:
#                    dist2 = ndimage.rotate(dist2+0.01,90,reshape=False)
#                elif randNum < 0.75:
#                    dist2 = ndimage.rotate(dist2+0.01,112.5,reshape=False)
#                elif randNum < 0.875:
#                    dist2 = ndimage.rotate(dist2+0.01,135,reshape=False)
#                elif randNum < 1.01:
#                    dist2 = ndimage.rotate(dist2+0.01,157.5,reshape=False)
#                else:
#                    print("error!")
#
#
...
#                self.blocks.append(np.reshape(im,int(self.blockSize**2)))
    ...
    def reset(self):
        ...
        if self.num_orientations == 4:
            self.orientationVector = [0,0,45,45,90,90,135,135]
...
#                    print("error!")
                ...
                if randNum < 0.125:
                    ...
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[1],reshape=False)
                elif randNum < 0.375:
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[2],reshape=False)
                elif randNum < 0.5:
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[3],reshape=False)
                elif randNum < 0.625:
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[4],reshape=False)
                elif randNum < 0.75:
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[5],reshape=False)
                elif randNum < 0.875:
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[6],reshape=False)
                elif randNum < 1.01:
                    dist2 = ndimage.rotate(dist2+0.01,self.orientationVector[7],reshape=False)
                else:
                    print("error!")
                ...
                if np.sum(self.state[0][iiRangeOuter,jjRangeOuter,0]) > 0:
                    ...
                    if (np.sum(im[iiGap1,jjGap1]) < 1) and (np.sum(im[iiGap2,jjGap2]) < 1):
                        ...
                        if orientation == 0:
                            ...
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[1], reshape=False)>0.5)
                        elif orientation == 2:
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[2], reshape=False)>0.5)
                        elif orientation == 3:
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[3], reshape=False)>0.5)
                        elif orientation == 4:
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[4], reshape=False)>0.5)
                        elif orientation == 5:
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[5], reshape=False)>0.5)
                        elif orientation == 6:
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[6], reshape=False)>0.5)
                        elif orientation == 7:
                            imRot = np.int32(ndimage.rotate(im, self.orientationVector[7], reshape=False)>0.5)
                        else:
            ...
            if self.state[1] != 0:
                ...
                if True not in (self.state[0][iiRangeOuter,jjRangeOuter,0] > 0): # if this square is empty
                    ...
                    if orientation == 0:
                        ...
                        imRot = ndimage.rotate(im, -self.orientationVector[1], reshape=False)
                    elif orientation == 2:
                        imRot = ndimage.rotate(im, -self.orientationVector[2], reshape=False)
                    elif orientation == 3:
                        imRot = ndimage.rotate(im, -self.orientationVector[3], reshape=False)
                    elif orientation == 4:
                        imRot = ndimage.rotate(im, -self.orientationVector[4], reshape=False)
                    elif orientation == 5:
                        imRot = ndimage.rotate(im, -self.orientationVector[5], reshape=False)
                    elif orientation == 6:
                        imRot = ndimage.rotate(im, -self.orientationVector[6], reshape=False)
                    elif orientation == 7:
                        imRot = ndimage.rotate(im, -self.orientationVector[7], reshape=False)
                    else:
...
#        allowedGapFraction = 0.7 # percent of total block size that fits between fingers
#        allowedGapFraction = 0.9 # percent of total block size that fits between fingers. This was not small enough to prevent all non-aligned arrangements...
        allowedGapFraction = 0.8 # percent of total block size that fits between fingers
        if self.state[1] == 0: # if both objs on the grid

            im = np.int32(self.state[0][:,:,0]>0.5)
            ...
            im = np.int32(ndimage.rotate(self.state[0][:,:,0],22.5,reshape=True)>0.5)            
            bounds = self.getBoundingBox(im)
            if (bounds[1] - bounds[0]) < (self.blockSize*allowedGapFraction):
                if (bounds[3] - bounds[2]) < (self.blockSize*3.0):
                    done = 1
            ...
            im = np.int32(ndimage.rotate(self.state[0][:,:,0],67.5,reshape=True)>0.5)            
            bounds = self.getBoundingBox(im)
            if (bounds[1] - bounds[0]) < (self.blockSize*allowedGapFraction):
                if (bounds[3] - bounds[2]) < (self.blockSize*3.0):
                    done = 1
