...
#        initMoveCenters = self.moveCenters
        ...
        self.state = []
        self.holdingImage = np.zeros([self.blockSize,self.blockSize])
    ...
    def step(self, action):
        ...
        actionPos = action % self.num_moves # position of target action
        actionRot = action / self.num_moves # orientation of target action
        ...
        # if PICK
        if actionPos < self.num_moves:
            
            # if not holding anything
            if self.state[1] == 0:
                ii = coords[0,actionPos]
                jj = coords[1,actionPos]
                iiRangeOuter, jjRangeOuter = np.meshgrid(range(ii-halfSide,ii+halfSide),range(jj-halfSide,jj+halfSide))
                
                if np.sum(self.state[0][iiRangeOuter,jjRangeOuter,0]) > 0: # if there's something there
                    im = self.state[0][iiRangeOuter,jjRangeOuter,0]
                    iiRangeInner, jjRangeInner = np.meshgrid(range(ii-halfSide+self.gap,ii+halfSide-self.gap),range(jj-halfSide,jj+halfSide))
                    if (np.sum(self.state[0][iiRangeOuter,jjRangeOuter,0]) - np.sum(self.state[0][iiRangeInner,jjRangeInner,0])) == 0:
                        self.holdingImage = np.copy(self.state[0][iiRangeOuter,jjRangeOuter,0])
                        self.state[1] = 1 # set holding to contents of action target
                        self.state[0][iiRangeOuter,jjRangeOuter,0] = np.zeros([len(iiRangeOuter),len(jjRangeOuter)])

            
        # if PLACE
        elif actionPos < 2*self.num_moves:
            
            actionPos -= self.num_moves
            
            if self.state[1] != 0:
                
                ii = coords[0,actionPos]
                jj = coords[1,actionPos]
                iiRangeInner, jjRangeInner = np.meshgrid(range(ii-halfSide,ii+halfSide),range(jj-halfSide+self.gap,jj+halfSide-self.gap))
                iiRangeOuter, jjRangeOuter = np.meshgrid(range(ii-halfSide,ii+halfSide),range(jj-halfSide,jj+halfSide))
