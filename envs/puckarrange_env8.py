    ...
    def step(self, action):
        ...
        if pickplace == 0:
            ...
            if self.state[1] == 0:
                ...
                if np.sum(self.state[0][iiRangeOuter,jjRangeOuter,0]) > 0: # if something there
                    if (np.sum(self.state[0][iiRangeInner,jjRangeInner,0]) - np.sum(self.state[0][iiRangeOuter,jjRangeOuter,0])) == 0: # if nothing in finger range
