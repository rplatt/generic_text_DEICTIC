...
#
# In this version, I incorporate build_getDeic_FocCoarse. This function gets a deictic coarse view
# as well as a focused view.
#
# Results: I get up to 0.9 on ballcatch using 3x3x4 focused/coarse deictic patches. This is better 
#          that I was able to do w/ either 3x3x2 or 4x4x2 focused patches. It also seems to learn
#          on the ghostevade domain.
#
#          Also, I notice that it's hard to get ghostevade to work well w/ just 3x3x2 patches. I think this
#          reflects the challenges of using the neural network. Actually, 3x3x4 seems to work better than 3x3x2.
#
#          Conclusions: 
#               -- The key result is that learning is a lot slower when using a 4-channel image 
#               rather than a 2-channel image. Not sure why this is -- could be related
#               to network structure.
#
#               -- Also, it seems that the fine/coarse patch can get better perfoance
#                  on the ballcatch domain (where it is needed).
#
import gym
import numpy as np
import time as time
...
def build_getDeic_Old(make_obs_ph,deicticShape):
...
# Focused-view-only representation
...
#    coarseAgent = tf.cast(tf.image.resize_area(tf.cast(tf.equal(patchesTiledLarge,1.),tf.int32), deicticShape[0:2]) > 0,tf.int32)
#    coarseGhost = tf.cast(tf.image.resize_area(tf.cast(tf.equal(patchesTiledLarge,2.),tf.int32), deicticShape[0:2]) > 0,tf.int32)
...
#    patchesTiledStacked = tf.stack([tf.equal(patchesTiledFocused,1), tf.equal(patchesTiledFocused,2)],axis=-1)
    ...
    getDeic = U.function(inputs=[observations_ph], outputs=patchesTiledStacked)
#    getDeic = U.function(inputs=[observations_ph], outputs=[obs, obsZeroPadded, patchesTiledLarge, patchesTiledFocused, coarseAgent, coarseGhost, patchesTiledStacked])
    return getDeic
...
#        return U.BatchInput([num_actions], name=name)
    ...
    getDeicFoc = build_getDeic_Old(make_obs_ph=make_obs_ph,deicticShape=deicticShape)
#    getDeicFoc = build_getDeic_Foc(make_obs_ph=make_obs_ph,deicticShape=deicticShape)
#    getDeicFocCoarse = build_getDeic_FocCoarse(make_obs_ph=make_obs_ph,deicticShape=deicticShape)
    ...
    for t in range(max_timesteps):
        ...
        obsDeictic = getDeicFoc([obs])
#        obsDeictic = getDeicFocCoarse([obs])
#        obsDeictic = getDeicFocCoarse([obs])[:,:,:,0:2]
...
#        action = np.argmax(np.max(qCurrNoise[:,0,:],0)) # DO NOT USE CASCADE
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            obses_t_deic = getDeicFoc(obses_t)
            obses_tp1_deic = getDeicFoc(obses_tp1)
#            obses_t_deic = getDeicFocCoarse(obses_t)
#            obses_tp1_deic = getDeicFocCoarse(obses_tp1)
#            obses_t_deic = getDeicFocCoarse(obses_t)[:,:,:,0:2]
#            obses_tp1_deic = getDeicFocCoarse(obses_tp1)[:,:,:,0:2]
