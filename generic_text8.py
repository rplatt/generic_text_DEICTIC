#
# Adapted from generic_text6.py. 
#
# In this version, I'm planning to implement a tabular version of the DL stuff
# using a python dictionary to encode value.
#
# I could not get this to work for some reason. I'm going to experiment with a 
# stripped down version in generic_text8_stripped.py
#
import gym
import numpy as np
import time as time
...
#        return U.BatchInput([num_actions], name=name)
    ...
    def getTabular(obsDeicticTiled):
        ...
        return np.array([q_func[x] if x in q_func else np.zeros([num_cascade,num_actions]) for x in keys])
...
#       TABULAR version
        qCurr = getTabular(np.reshape(obsDeictic,[-1,deicticShape[0]*deicticShape[1]*deicticShape[2]]))
...
#            obses_tp1_deic = getDeic(obses_tp1)[:,:,:,0:2]
            ...
            qNext = getTabular(np.reshape(obses_tp1_deic,[-1,deicticShape[0]*deicticShape[1]*deicticShape[2]]))
            qCurr = getTabular(np.reshape(obses_t_deic,[-1,deicticShape[0]*deicticShape[1]*deicticShape[2]]))
...
#                qCurrTargets[range(expLen),i,actionsTiled] = targets
                qCurrTargets[range(expLen),i,actionsTiled] = np.minimum(qCurrTargets[range(expLen),i,actionsTiled],targets)
...
#                    (1-mask)*qCurr[range(expLen),i+1,actionsTiled]
            ...
            # TABULAR version
            trainTabular(np.reshape(obses_t_deic,[-1,deicticShape[0]*deicticShape[1]*deicticShape[2]]),qCurrTargets)
