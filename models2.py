# Derived from models.py
#
# In this version, I'm creating a new version of cnn_to_mlp with two pathways.
#
...
def _cnn_to_mlp_2pathways(convs, hiddens, dueling, inpt, num_actions, scope, reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        out1 = inpt[:,:,:,0:2]
        out2 = inpt[:,:,:,2:4]
        with tf.variable_scope("convnet"):
            ...
            for num_outputs, kernel_size, stride in convs:
                out2 = layers.convolution2d(out2,
                                           ...
                                           activation_fn=tf.nn.relu)
#        out = layers.flatten(out)
        out = tf.concat([layers.flatten(out1), layers.flatten(out2)],1)
...
def cnn_to_mlp_2pathways(convs, hiddens, dueling=False):
    return lambda *args, **kwargs: _cnn_to_mlp_2pathways(convs, hiddens, dueling, *args, **kwargs)
