#
# Hi-res version of numbersarrange1.py
#
# Adapted from numbersarrange1.py
#
# Results: With descriptorShapeSmall = (10,10,2), 1000/10 works well for step=28,14 
#          but not as well for step=20.
#          However, if you increase descriptorShapeSmall = (20,20,2), step=20 does work well.
#          I tried descriptorShapeSmall = (20,20,2) w/ step=7, and that worked well.
#          I've been looking at the value functions, and I think that the interaction
#          between descriptorshape and step can cause weird patterns in the value
#          function.
#
#          I also think that the exact numeral selected can have a significant effect
#          on how well it is detected.
#
#          -- 1000/10, descriptorShapeSmall = (15,15,2) works better than (10,10,2)
#             for stride=14 and prune=False. It also works for stride=7 w/ prune=True, 
#             but it's a bit more multimodal in the value function.
#
#          -- 1000/10, descriptorShapeSmall = (20,20,2), failed for stride=7 (either w/ prune=True or False)
#
#          -- 1000/10, descriptorShapeSmall = (20,20,2) worked well for stride=3
#
# Conclusion: You need to have a descriptorShapeSmall that is large enough to support
#             the step resolution you're working with.
#
import gym
import numpy as np
import time as time
...
#    descriptorPrune = True
    descriptorPrune = False
...
#    getMoveActionDescriptors = build_getMoveActionDescriptors(make_obs_ph=make_obs_ph,actionShape=actionShape,actionShapeSmall=actionShapeSmall)
    ...
    for t in range(max_timesteps):
        ...
        # Get action descriptors -- pick followed by place actions
        ...
        # Prune duplicate descriptors
        if descriptorPrune:
            _,descriptorsIdx,descriptorsInv = np.unique(actionDescriptors,axis=0,return_index=True,return_inverse=True)
            actionDescriptorsPruned = actionDescriptors[descriptorsIdx]
        else:
            actionDescriptorsPruned = actionDescriptors
            
        # Get qCurr values
        if valueFunctionType == "TABULAR":
            actionDescriptorsFlat = np.reshape(actionDescriptorsPruned,[-1,actionShape[0]*actionShape[1]*actionShape[2]]) == 1
            qCurr = getTabular(actionDescriptorsFlat)
        else:
            qCurrNotHolding = getqNotHolding(actionDescriptorsPruned)
            qCurrHolding = getqHolding(actionDescriptorsPruned)
            qCurr = np.concatenate([qCurrNotHolding,qCurrHolding],axis=1)

        # If pruned, then re-expand values
        if descriptorPrune:
            qCurr = qCurr[descriptorsInv]
        ...
        if actionSelectionStrategy == "UNIFORM_RANDOM":
            ...
            if not descriptorPrune:
                _,descriptorsIdx,descriptorsInv = np.unique(actionDescriptors,axis=0,return_index=True,return_inverse=True)
            actionIdx = np.argmax(qCurrNoise[descriptorsIdx,obs[1]])
            if np.random.rand() < exploration.value(t):
                actionIdx = np.random.randint(len(descriptorsIdx))
            actionsSelected = np.nonzero(descriptorsInv==actionIdx)[0]
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            if descriptorPrune:
                actionDescriptorsPruned,_,descriptorsNextInv = np.unique(actionDescriptorsNext,axis=0,return_index=True,return_inverse=True)
            else:
                actionDescriptorsPruned = actionDescriptorsNext
            ...
            if valueFunctionType == "TABULAR":
                ...
                qNextNotHolding = getqNotHolding(actionDescriptorsPruned)
                qNextHolding = getqHolding(actionDescriptorsPruned)
                qNextFlat = np.concatenate([qNextNotHolding,qNextHolding],axis=1)

            # If pruned, then re-expand values
            if descriptorPrune:
                qNextFlat = qNextFlat[descriptorsNextInv]
