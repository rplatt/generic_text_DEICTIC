#
# Checkpoint. This version only calculates values for actual current state. Also
# this version calculates the NoRot value function (but does not use it yet).
#
...
#    prioritized_replay=True
    ...
    for t in range(max_timesteps):
        ...
        # Prune Rot descriptors
        k=0.1 # top 10%
        valsNoRot = qCurrNoRot
        topKelts = np.argsort(valsNoRot)[-np.int32(np.shape(valsNoRot)[0]*k):]
