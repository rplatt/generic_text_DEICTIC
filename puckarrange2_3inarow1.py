#
# This version incorporates a place memory to enable it to handle placing three
# pucks in a row.
#
# Adapted from puckarrange2.py
#
# Results: this does not seem to converge consistently for three blocks. I
#          use batch_size=1, buffer_size=1. But, when it did converge, it remained
#          converged...
#
#
import sys as sys
...
#    placeMemory = np.zeros([1, descriptorShapeSmall[0], descriptorShapeSmall[1], 1])
    ...
    for t in range(max_timesteps):
        ...
        # Augment descriptors w/ place memory
        placeMemoryTiled = np.repeat([placeMemory],num_patches,axis=0)
        actionsPickDescriptors = np.concatenate([actionsPickDescriptors,placeMemoryTiled],axis=3)
        actionsPlaceDescriptors = np.concatenate([actionsPlaceDescriptors,placeMemoryTiled],axis=3)
...
#        # check how well V is being estimated
#        if obs[1]:
#            Vest = getVHolding([placeMemory])
#        else:
#            Vest = getVNotHolding([placeMemory])
#        print("V comparison: " + str(Vest) + ", " + str(thisStateValues))
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            # Calculate target (binary state)
#            targets = rewards + (1-dones) * gamma * V[states_tp1]

# HERE'S WHERE I LEFT OFF (BELOW). NEXT STEP IS TO TEST NEW TARGET ESTIMATE IN CL RL.
