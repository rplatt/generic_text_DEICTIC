#
# Adding an entry for placeMemory
#
# derived from replay_buffer8.py
#
import numpy as np
import random
import copy as cp
...
class ReplayBuffer(object):
    ...
    def add(self, stateDiscrete_t, actionImage_t, reward, stateDiscrete_tp1, placeMemory_tp1, done):
        data = (stateDiscrete_t, actionImage_t, reward, stateDiscrete_tp1, placeMemory_tp1, done)
    ...
    def _encode_sample(self, idxes):
        statesDiscrete_t, actionImages_t, rewards, statesDiscrete_tp1, placeMemories_tp1, dones = [], [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
            stateDiscrete_t, actionImage_t, reward, stateDiscrete_tp1, placeMemory_tp1, done = data
            ...
            placeMemories_tp1.append(np.array(placeMemory_tp1, copy=False))
            dones.append(done)
        return np.array(statesDiscrete_t), np.array(actionImages_t), np.array(rewards), np.array(statesDiscrete_tp1), np.array(placeMemories_tp1), np.array(dones)
