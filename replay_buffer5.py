# 
# This version of replay buffer is adapted to accomodate adjacency information.
#
...
#    def add(self, obs_t, action, reward, obs_tp1, done):
    def add(self, obs_t, action, adjacentActions, reward, obs_tp1, done):
#        data = (obs_t, action, reward, obs_tp1, done)
        data = (obs_t, action, adjacentActions, reward, obs_tp1, done)
...
#        states_t, actions, rewards, states_tp1, images_tp1, dones = [], [], [], [], [], []
        states_t, actions, adjacentActions, rewards, states_tp1, images_tp1, dones = [], [], [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
#            state_t, action, reward, obses_tp1, done = data
            state_t, action, adjacentAction, reward, obses_tp1, done = data
            states_t.append(np.array(state_t, copy=True))
            actions.append(np.array(action, copy=True))
            adjacentActions.append(np.array(adjacentAction, copy=True))
            rewards.append(reward)
            images_tp1.append(np.array(obses_tp1[0], copy=True))
            states_tp1.append(np.array(obses_tp1[1], copy=True))
            dones.append(done)
#        return np.array(states_t), np.array(actions), np.array(rewards), np.array(images_tp1), np.array(states_tp1), np.array(dones)
        return np.array(states_t), np.array(actions), np.array(adjacentActions), np.array(rewards), np.array(images_tp1), np.array(states_tp1), np.array(dones)
